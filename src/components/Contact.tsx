    ...
    return (
        <section id="contact" className="flex flex-col items-center mb-80 pt-12 scroll-mt-24">
            ...
            <form onSubmit={submit} action="#contact" method="POST" className="flex flex-col px-4 mt-5 w-full">
                <input type="text" name="name" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} className="bg-light-blue text-white rounded-2xl w-full h-16 pl-3"/>
                <label className="pl-3">{nameError && <p className="text-red opacity-80">{nameError}</p>}</label>
                <textarea name="message" placeholder="Message" rows={10} value={message} onChange={(e) => setMessage(e.target.value)} className="bg-light-blue text-white rounded-2xl w-full pl-3 pt-3 mt-3 resize-none"></textarea>
                ...
                <div className="flex justify-end">
                    ...
                    {success && <div className="flex justify-center items-center bg-lime-600 opacity-80 text-white rounded-2xl w-full h-10 mt-3 text-lg cursor-pointer">Thank you!</div>}
                </div>
            </form>
        </section>
    );
